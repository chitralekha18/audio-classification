...
# test_spectset2snd_d1.9_d0.7_v.0.wav
csvfile = 'GAN.csv'
fout = open(csvfile,'w')
fout.write('filename,fold,target,inst,pitch\n')
folder = '2021.04.26_nsynthsubset_edgepinned'
filenamelist = []
target = []
pitch = []
inst = []
for file in glob.glob(folder+'/*.wav'):
	...
	inst_id = int(filename.split('_')[-3].split('.')[1])
	if inst_id in range(0,7):
		target.append('0')
	elif inst_id in range(7,14):
		target.append('1')
	else:
		target.append('2')
	
	pitch.append(filename.split('_')[-2].split('.')[1])
	inst.append(str(inst_id))
